...
5. использовать токен клиента (номер) в медицинских журналах
...
flowchart TD    
    ...
    resep --> pr_register
    pr_register -->|Записывает общие данные <br> в список пациентов <br> TLS| D1
...
flowchart TD    
    ...
    resep -->|Замена ФИО на токен| pr
    pr -->|Записывает журнал <br> PHI <br> TLS| DP
...
flowchart TD    
    ...
    client -->|"Сообщает ФИО <br> симптомы <br> PII:base, PII:sense"| doc 
    doc -->|Получает токен <br> из журнала приема| prp
    prp --> pra
...
flowchart TD    
    ...
    client -->|"Сообщает ФИО <br> PII:base"| pers 
    pers -->|Получает токен <br> из журнала приема| pr
    pr --> kkm
